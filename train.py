...
class ImageTrainer():
    ...
    def coin_train(self, idx):
        ...
        if self.state in ["train", "mtrain"]:                                               ##! 训练模式
            ...
            for step, img_data in enumerate(self.dataset):
                ...
                each_psnr, each_ssim, each_lpips = self.training(train_net, img_data)
            ...
            with tqdm(total=self.args.out_epochs*self.img_num) as pbar:
                for out_epoch in range(self.args.out_epochs):
                    for step, img_data in enumerate(self.meta_dataset):
                        ...
                        #     print("save last mod")
                        #     torch.save(meta_net.net.state_dict(), os.path.join(meta_net.mods_meta_path, f"meta_w{c_width}d{c_depth}.pth"))                  
        if self.state in ["quant"]:                                                         ##! 量化熵编码模式                                               
        ...
        #     get_res(self.args, idx, self.args.img_name)
            ...
            for step, img_data in enumerate(self.dataset):
                ...
                if self.state == "train":
                    ...
                    if idx==0:
                        ...
                        train_net.load_net(sub_net, s_width)
                        train_net.prune_net(s_width, True, True)
                each_psnr, each_ssim, each_lpips = self.training(train_net, img_data, s_width)
                all_psnrs.append(each_psnr)
                if self.eval_all:
                    all_ssims.append(each_ssim)
                    all_lpips.append(each_lpips)
    ...
    def sd_train(self, idx):
            ...
            for step, img_data in enumerate(self.dataset):
                ...
                if self.state == "train":
                    ...
                    if idx==0:
                        ...
                        train_net.load_net(sub_net, s_width, s_depth)
                        train_net.init_net(s_depth, True, True, 30.*sc[idx], sc[idx])
                each_psnr, each_ssim, each_lpips = self.training_multi(train_net, img_data, c_width, c_depth, s_width, s_depth)
                all_psnrs.append(each_psnr)
                if self.eval_all:
                    all_ssims.append(each_ssim)
                    all_lpips.append(each_lpips)
        ...
        if self.state in ["quant"]:
            quantizer = Quentizer(self.args)
            ori_psnr_list, quant_psnr_list, down_psnr_list = [], [], []     ## [[1, 2, 3], [4, 5, 6]]
            entropy_list = []
            for num_bit in self.args.num_bits:
                print("num_bit:", num_bit)
                ori_psnr_bit, quant_psnr_bit, down_psnr_bit = [], [], []    ## [1, 2, 3]
                entropy_bit = []
                for step, img_data in enumerate(self.dataset):
                    name = img_data["name"]
                    if self.args.img_name != None:
        ...
        if args.plot_distribution:
            for step, img_data in enumerate(self.dataset):
                ...
                # sc = math.sqrt((st_c + st_m) / (st_m * c_width))
                if idx > 0:
                    net.init_net(s_depth, True, True, 30.*sc[idx], sc[idx])
                ## 获取刚初始化的权值
                param_list = []
            ...
            for step, img_data in enumerate(self.dataset):
                ...
                if self.state == "train":
                    ...
                    if (c_depth-s_depth)>0 and idx>0:
                        train_net.prune_net(s_width, s_depth, True, True)
                each_psnr, each_ssim, each_lpips = self.training_multi(train_net, img_data, c_width, c_depth, s_width, s_depth, True)
                all_psnrs.append(each_psnr)
                if self.eval_all:
                    all_ssims.append(each_ssim)
                    # all_lpips.append(each_lpips)
    ...
    """ COIN 和 SW 训练过程 """
    def training(self, train_net, img_data, s_width=0):
        train_net.train()
        ...
        best_psnr, best_ssim, best_lpips = 0, 0, 0
        with tqdm(total=self.args.epochs) as pbar:
            for epoch in range(1, self.args.epochs+1):
                ...
                if epoch and (not epoch % self.logs_inter):
                    ...
                    eval_width, eval_depth = train_net.c_width, train_net.c_depth
                    ...
                    if self.eval_all:
                        print(f"Epoch[{epoch}/{self.args.epochs}]\tPSNR:{best_psnr}\tSSIM:{best_ssim}")
                    else:
    ...
    def training_multi(self, train_net, img_data, c_width, c_depth, s_width=0, s_depth=0, last_loss=True):
        train_net.train()
        optimize = torch.optim.Adam(train_net.parameters(), self.args.lr, amsgrad=True)
        coord, feature, H, W = img_data["coord"], img_data["feature"], img_data["H"], img_data["W"]
        each_psnr, each_ssim, each_lpips = [], [], []
